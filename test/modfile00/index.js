...
describe('Modify a File with two conflicts', function () {
    var repoPath,
        ...
        rebase0Path = path.join(__dirname, 'rebase0'),
        rebase1Path = path.join(__dirname, 'rebase1'),
        git;
    ...
    it('should reach the rebase state', function (done) {
        testUtils.assertDifferent(repoPath, rebase0Path, '.git').then(function () {
            done();
        }).catch(function (err) {
            done(err);
        });
    ...
    describe('resolve conflicts and try to terminate', function () {
        ...
        it('should reach the second rebase state', function (done) {
            testUtils.assertDifferent(repoPath, rebase1Path, '.git').then(function () {
                done();
            }).catch(function (err) {
                done(err);
            });
        ...
        describe('resolve conflicts and try to terminate', function () {
            ...
            beforeEach(function (done) {
                rm('a.txt').then(function () {
                    return fs.copy(path.join(finalPath, 'b.txt'), path.join(repoPath, 'b.txt'));
                }).then(function () {
                    return git.add('-A');
                }).then(function () {
                    return commands.evolve.end(repoPath);
